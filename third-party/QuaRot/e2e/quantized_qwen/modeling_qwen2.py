...
from e2e.checkpoint_utils import hadamard_utils
...
class Qwen2MLP(nn.Module):
    ...
    def __init__(
        ...
        # )
        self.gate_up_proj = quarot.nn.Linear4bit(
...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self,
        ...
        self.qkv_proj = quarot.nn.Linear4bit(
            in_features=hidden_size,
        ...
        self.o_proj = quarot.nn.Linear4bit(
    ...
    def o_had_trans(self, x):
        init_shape = x.shape
        if self.o_K == 1:
            x = fast_hadamard_transform.hadamard_transform(x.reshape(-1, init_shape[-1]//self.o_had_dim, self.o_had_dim).transpose(1, 2),
                                                            scale=1/math.sqrt(init_shape[-1]//self.o_had_dim)).transpose(1, 2)
        else:
            # had_k to o_hadK
            x = (self.o_hadK.to(x.device, x.dtype) @ x.reshape(-1, init_shape[-1]//self.o_had_dim, self.o_had_dim)) / math.sqrt(init_shape[-1]//self.o_had_dim)
...
class Qwen2Model(nn.Module):
    ...
    def __init__(self, *, vllm_config, prefix: str = ""):
        ...
        cache_config = None # vllm_config.cache_config
        quant_config = None # vllm_config.quant_config
        ...
        if get_pp_group().is_last_rank:
            self.norm = quarot.nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps,fuse=False)
        else:
