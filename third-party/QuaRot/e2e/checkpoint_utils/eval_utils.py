...
def evaluator(model, testenc, dev, args):
    ...
    #     opt_type = True
    #     llama_type = False
    # elif 'meta' in args.model or 'llama' in args.model.lower():
    #     llama_type = True
    #     opt_type = False
    # else:
    ...
    class Catcher(torch.nn.Module):
        ...
        def forward(self, inp, **kwargs):
            ...
            if llama_type:
                ...
                #cache['past_key_value'] = kwargs['past_key_value']
            raise ValueError
    ...
    for i in tqdm(range(len(layers)), desc="(Eval) Layers"):
        ...
        for j in range(nbatches):
            if opt_type:
                ...
                if i == 0:
                    past_key_value = quarot.transformers.MultiLayerPagedKVCache4Bit(
                        batch_size=inps[j].shape[0],
                        page_size=inps[j].shape[1], 
                        max_seq_len=inps[j].shape[1],
                    ...
                    past_key_values[j] = past_key_values[j].to(torch.device('cuda:0'))
                outs[j] = layer(inps[j], attention_mask=attention_mask, position_ids=position_ids, past_key_value=past_key_values[j])[0]
                past_key_values[j] = past_key_values[j].to(torch.device('cpu'))
        layers[i] = layer.cpu()
        del layer
        torch.cuda.empty_cache()
        inps, outs = outs, inps
    ...
    nlls_tensor = nlls_tensor[~torch.isnan(nlls_tensor)]
