...
from quarot import fuse_asym_quantize_and_pack_i4
...
class MultiLayerPagedKVCache4Bit(Cache):
    def __init__(
        ...
        # cache for kv_indptr and kv_indices
        # -------------------------------------
        self.kv_indptr = torch.arange(0, batch_size + 1, device=device, dtype=torch.int) * max_page_cnt
        self.page_cnt = max_page_cnt
        self.kv_indices = (
            (torch.arange(self.page_cnt, device=self.device, dtype=torch.int) * self.batch_size).unsqueeze(0) + 
            torch.arange(self.batch_size, device=self.device, dtype=torch.int).unsqueeze(1)
        ).view(-1)
        self.page_ptr = torch.full((self.batch_size, ), self.page_size, device=self.device, dtype=torch.int)
        # -------------------------------------
        # end of cache for kv_indptr and kv_indices
    ...
    def update(
        ...
        if self.disable_quant:
            ...
            key_states,value_states, k_scale, k_zero, v_scale, v_zero = fuse_asym_quantize_and_pack_i4(key_states, value_states)
        ...
        if self._needs_init[layer_idx]:
            ...
            kw_args = self.get_cache_specs_for_flash_infer(attention_mask)
            append_kv(
                **kw_args,
    ...
    def get_cache_specs_for_flash_infer(self, attention_mask):
        if attention_mask is not None:
            ...
            # TODO Whether can we use attention_mask directly to replace the page_ptr(which is actually some kind of mask)
        else:
            # seqlens_in_batch = torch.tensor([self.length], dtype=torch.int32, device=self.device).expand(self.batch_size)
            page_cnt = (self.length + self.page_size - 1) // self.page_size
            page_ptr = self.length % self.page_size
            
            if page_ptr == 0 and self.length != 0:
                #self.page_ptr = torch.zeros((self.batch_size, ), device=self.device, dtype=torch.int)
                page_ptr = torch.full((self.batch_size, ), self.page_size, device=self.device, dtype=torch.int)
            else:
                #self.page_ptr = self.page_ptr + 1
                page_ptr = torch.full((self.batch_size, ), page_ptr, device=self.device, dtype=torch.int)
                
            if self.page_cnt != page_cnt:
                self.kv_indptr = self.kv_indptr // self.page_cnt * page_cnt
                self.kv_indices = (
            ...
            return {
                ...
                f"kv_indptr": self.kv_indptr, 
                f"kv_indices": self.kv_indices, 
                f"last_page_offset": page_ptr, 
