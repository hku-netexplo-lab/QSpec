...
def main():
    ...
    if args.rotate:
        ...
        for name in qlayers:
            if 'down_proj' in name:
                ...
                qlayers[name].online_full_had = True #True
                qlayers[name].had_K = had_K
                qlayers[name].K = K
                qlayers[name].fp32_had = args.fp32_had
            if 'o_proj' in name:
                had_K, K = hadamard_utils.get_hadK(model.config.num_attention_heads)
                qlayers[name].online_partial_had = True #True
                qlayers[name].had_K = had_K
                qlayers[name].K = K
    ...
    if args.w_bits < 16:
        ...
        if args.load_qmodel_path: # Load Quantized Rotated Model
            ...
            assert "llama" in args.model or 'Llama' in args.model, "Only llama is supported for GPTQ!"
