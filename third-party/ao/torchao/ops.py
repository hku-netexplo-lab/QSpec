...
lib.define(
    "rowwise_scaled_linear_cutlass_s4s4_unified(Tensor input, Tensor input_scale, Tensor weight, Tensor weight_scale, Tensor bias, Tensor result) -> Tensor"
)
...
def rowwise_scaled_linear_cutlass_s4s4_unified(
    input: Tensor,
    input_scale: Tensor,
    weight: Tensor,
    weight_scale: Tensor,
    ...
    """
    CUTLASS-based row-wise scaled W8A4 linear operator.
    Args:
        input: quantized input tensor, in row-major layout.
        input_scale: scale factors for input tensor, has to be tensor of the same shape as the input tensor, minus the last dimension.
        weight: quantized weight matrix, in row-major layout.
        weight_scale: scale factors for weight tensor, one value per row of weight matrix (thus also tensor of the same shape as the weight tensor, minus the last dimension).
        bias: a vector of size equal to number of rows of weight tensor, or None.
        result: output tensor, in row-major layout.
    Returns:
        output: result tensor, in row-major layout.
    """

    return torch.ops.torchao.rowwise_scaled_linear_cutlass_s4s4_unified.default(
        input, input_scale, weight, weight_scale, bias, result
    )



@register_custom_op("torchao::rowwise_scaled_linear_cutlass_s4s4_unified")
def _(
    input: Tensor,
    input_scale: Tensor,
    weight: Tensor,
...
# def rowwise_scaled_linear_cutlass_awq_unified(
#     input: Tensor,
#     input_scale: Tensor,
#     weight: Tensor,
#     weight_scale: Tensor,
...
#     return torch.ops.torchao.rowwise_scaled_linear_cutlass_awq_unified.default(
#         input, input_scale, weight, weight_scale, bias, result
#     )



# @register_custom_op("torchao::rowwise_scaled_linear_cutlass_awq_unified")
# def _(
#     input: Tensor,
#     input_scale: Tensor,
#     weight: Tensor,
