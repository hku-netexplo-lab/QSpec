...
template <typename T>
__device__ T clamp_int4(T value)
{
    return fmax(fmin(value, 7.f), -8.f);
}

// from TRTLLM
template <typename Tf, typename T>
__inline__ __device__ Tf compute_layernorm_no_gamma(Tf val, float s_mean, float s_variance, const T* beta, int i)
{
...
template <typename T, typename scale_type, bool USE_DIFF_OF_SQUARES = false>
__global__ void generalLayerNorm_fuse_sum_i8(const T* input, const T* beta, T* normed_output, const float eps,
    int tokens, int hidden_dim, scale_type* input_sum, const scale_type* scale_orig_quant_per_tensor, scale_type* scale_orig_quant_per_token,
    int8_t* normed_output_quant, bool use_shmem)
{
    constexpr auto num_elems_T = num_elems<T>::value;
    ...
    for (int i = tidx; i < n_elems; i += blockDim.x)
    {
        const int index = bidx * n_elems + i;
        const float_packed_t val_f = cuda_cast<float_packed_t>(use_shmem ? shmem[i] : input[index]);
        const T val = cuda_cast<T>(compute_layernorm_no_gamma(val_f, s_mean, s_variance, beta, i));
    ...
    if (with_per_token_scaling)
    {
        ...
        for (int i = tidx; i < n_elems; i += blockDim.x)
        {
            ...
            if (!use_shmem)
            {
                val_f = compute_layernorm_no_gamma(val_f, s_mean, s_variance, beta, i);
            }
...
template <typename T, typename scale_type, bool USE_DIFF_OF_SQUARES = false>
__global__ void generalLayerNorm_fuse_sum_i4(const T* input, const T* beta, T* normed_output, const float eps,
    int tokens, int hidden_dim, scale_type* input_sum, const scale_type* scale_orig_quant_per_tensor, scale_type* scale_orig_quant_per_token,
    int8_t* normed_output_quant, bool use_shmem)
{
    constexpr auto num_elems_T = num_elems<T>::value;
    ...
    if (with_per_token_scaling)
    {
        float abs_max_f = blockAllReduceMax(cuda_cast<float>(amax));
        float sum_f = blockAllReduceSum(cuda_cast<float>(sum));
        const float dynamic_per_token_scale = 7.f / abs_max_f;
        for (int i = tidx; i < n_elems; i += blockDim.x)
        {
            int index = bidx * n_elems + i;
            float_packed_t val_f = cuda_cast<float_packed_t>(use_shmem ? shmem[i] : input[index]);
            ...
            int8_packed_t clamped_pack = cuda_cast<int8_packed_t>(clamp_int4(val_f * cuda_cast<float_packed_t>(dynamic_per_token_scale)));

            if (i % 2 == 0)
            {
                float_packed_t val_f_next = cuda_cast<float_packed_t>(use_shmem ? shmem[i+1] : input[index+1]);
                if (!use_shmem)
                {
                    val_f_next = compute_layernorm_no_gamma(val_f_next, s_mean, s_variance, beta, i+1);
                }
                int8_packed_t clamped_pack_next = cuda_cast<int8_packed_t>(clamp_int4(val_f_next * cuda_cast<float_packed_t>(dynamic_per_token_scale)));
                reinterpret_cast<int8_t*>(normed_output_quant)[index/2] = (clamped_pack & 0x0F) | (clamped_pack_next & 0x0F) << 4;
            }

        }
        if (tidx == 0)
        {
            scale_orig_quant_per_token[bidx] = abs_max_f / 7.f;
            input_sum[bidx] = sum_f;
        }
    }
}
...
void rms_norm_general_fuse_sum_fp16(torch::Tensor &out,    // [..., hidden_size]
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "generalLayerNorm_fuse_sum_i4", [&] {
    ...
    // reinterpret_cast<T*>(weight.data_ptr<scalar_t>()), 
    nullptr,
    reinterpret_cast<T*>(out.data_ptr<scalar_t>()),  epsilon, num_tokens, hidden_size, nullptr, nullptr, nullptr,
    nullptr, false
    );
...
void rms_norm_general_fuse_sum_i8(torch::Tensor &out,    // [..., hidden_size]
              torch::Tensor &input,  // [..., hidden_size]
            //   torch::Tensor &weight, // [hidden_size]
              torch::Tensor &input_sum, // [tokens] or [1]
              torch::Tensor &scaling, // [tokens] or [1]
  ...
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "generalLayerNorm_fuse_sum_i8", [&] {
    using T = typename FloatTypeConverter<scalar_t>::Type;
    if (use_per_token_quant) {
      // per-token
      int shared_mem_size = block.x  * sizeof(T);
      vllm::generalLayerNorm_fuse_sum_i8<T, at::Half><<<grid, block, shared_mem_size, stream>>>(
        reinterpret_cast<T*>(input.data_ptr<scalar_t>()), 
        // reinterpret_cast<T*>(weight.data_ptr<scalar_t>()), 
        nullptr,
        nullptr, epsilon, num_tokens, hidden_size, input_sum.data_ptr<at::Half>(), nullptr, scaling.data_ptr<at::Half>(),
