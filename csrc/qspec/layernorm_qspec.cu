...
__global__ void
rms_norm_kernel_qspec(out_type *__restrict__ out,         // [..., hidden_size]
...
void rms_norm_qspec(torch::Tensor &out,    // [..., hidden_size]
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "rms_norm_kernel_qspec", [&] {
    if (use_quant) {
      vllm::rms_norm_kernel_qspec<scalar_t, int8_t, true><<<grid, block, 0, stream>>>(
        out.data_ptr<int8_t>(), input.data_ptr<scalar_t>(),
        weight.data_ptr<scalar_t>(), epsilon, num_tokens, hidden_size);
    } else {
      vllm::rms_norm_kernel_qspec<scalar_t, scalar_t, false><<<grid, block, 0, stream>>>(
...
void rms_norm_general_qspec(torch::Tensor &out,    // [..., hidden_size]
              torch::Tensor &input,  // [..., hidden_size]
              torch::Tensor &weight, // [hidden_size]
