...
#define OPERATOR_NAME_UNIFIED "rowwise_scaled_linear_cutlass_unified"
...
template<
    ...
    int NumStages,
    typename ElementA,  // FP16
    typename ElementB,  // int8 (packed int4)
    typename ElementOutput,  // FP16 or FP32
    typename ElementC,
    typename UseTensorC,
    typename ElementBScale>  // FP16 or FP32
void w4a16_per_channel_linear_kernel_cutlass_sm8x(
    const at::Tensor& tensor_a,  // FP16, shape [batch_size, in_features]
    const at::Tensor& tensor_b,  // int8, shape [out_features, in_features // 2]
    const at::Tensor& tensor_b_scale,  // FP16 or FP32, shape [out_features]
    const at::Tensor& tensor_c,  // FP16 or FP32, shape [batch_size, out_features]
    at::Tensor& tensor_d) {  // Output, shape [batch_size, out_features]
  static_assert((cutlass::sizeof_bits<ElementA>::value == 16) &&
                (cutlass::sizeof_bits<ElementB>::value == 8),
                "ElementA must be FP16, ElementB must be int8.");
  ...
  using ElementAccumulator = float;  // Use FP32 for accumulation
  using Operator = cutlass::arch::OpMultiplyAddMixedInputUpcast;

  using ElementEpilogue = float;  // Epilogue uses FP32

  constexpr auto NumEVTEpilogueStages = 1;

  const int m = tensor_a.size(0);  // batch_size
  const int n = tensor_b.size(0);  // out_features
  const int k = tensor_a.size(1);  // in_features

  constexpr int AlignmentA = 128 / cutlass::sizeof_bits<ElementA>::value;  // 8 for FP16
  constexpr int AlignmentB = 128 / cutlass::sizeof_bits<ElementB>::value;  // 16 for int8
  constexpr int AlignmentOutput = 128 / cutlass::sizeof_bits<ElementOutput>::value;

  // Check alignments
  TORCH_CHECK(k % AlignmentA == 0, "Number of columns of tensor A must be divisible by ", AlignmentA);
  TORCH_CHECK(k % AlignmentB == 0, "Number of columns of tensor B must be divisible by ", AlignmentB);
  TORCH_CHECK(n % AlignmentOutput == 0, "Number of columns of tensor C must be divisible by ", AlignmentOutput);
  ...
  // Only scalesb is used (Per-Channel scaling for weights)
  using TensorBScale =
  ...
  using EVTKernel = vllm::enable_2x_kernel_for_sm80_or_later<
  ...
  CUTLASS_STATUS_CHECK(status, "w4a16_per_channel_linear_kernel_cutlass_sm8x");
...
void rowwise_scaled_linear_kernel_cutlass_sm8x_unified(
  ...
  TORCH_CHECK(k % AlignmentA == 0, OPERATOR_NAME_UNIFIED,
              " : Number of columns of tensor A must be divisible by ",
              AlignmentA);
  TORCH_CHECK(k % AlignmentB == 0, OPERATOR_NAME_UNIFIED,
              " : Number of columns of tensor B must be divisible by ",
              AlignmentB);
  TORCH_CHECK(n % AlignmentC == 0, OPERATOR_NAME_UNIFIED,
  ...
  // by CUTLASS.
  status = gemm_op.can_implement(arguments);
  CUTLASS_STATUS_CHECK(status, OPERATOR_NAME_UNIFIED);
...
static void select_config_unified(
    const at::Tensor& tensor_a, const at::Tensor& tensor_a_scale,
    const at::Tensor& tensor_b, const at::Tensor& tensor_b_scale,
    const at::Tensor& tensor_c, at::Tensor& tensor_d) {
  ...
  if (is_sm8x) {
    if constexpr (std::is_same<ElementA, cutlass::int4b_t>::value &&
      ...
      if (tensor_a.size(0) <= 16) {
        ...
        rowwise_scaled_linear_kernel_cutlass_sm8x_unified<
  ...
  TORCH_CHECK(false, OPERATOR_NAME_UNIFIED, " : Operator not supported on SM",
...
dispatch_on_tensor_c_unified(
  ...
  TORCH_CHECK(false, OPERATOR_NAME_UNIFIED, " : Operator not supported for datatype ",
...
dispatch_on_tensor_a_scale_and_tensor_b_scale_unified(
  ...
  if (!tensor_a_scale.defined()){
    ...
    dispatch_on_tensor_c_unified<ElementA, ElementB, ElementOutput, ElementAScale,
      ElementBScale>(
  ...
  TORCH_CHECK(tensor_d.scalar_type() == tensor_a_scale.scalar_type(),
              OPERATOR_NAME_UNIFIED, " : Operator not supported for output datatype ",
  ...
  TORCH_CHECK(false, OPERATOR_NAME_UNIFIED,
...
template<typename ElementA, typename ElementB>
void
rowwise_scaled_linear_cutlass_check_inputs_unified(
  ...
  TORCH_CHECK(xq.dim() >= 2, OPERATOR_NAME_UNIFIED,
              " : Expected xq argument to be 2D or higher-dimensional tensor, "
              "got ", xq.dim(), " dims");
  TORCH_CHECK(xq.layout() == at::Layout::Strided, OPERATOR_NAME_UNIFIED,
              " : Expected xq argument to be strided, got layout ",
              xq.layout());
  TORCH_CHECK(x_scale.dim() == xq.dim() - 1, OPERATOR_NAME_UNIFIED,
              " : Expected xq scale argument to be ", xq.dim() - 1,
              "D tensor, got ", x_scale.dim(), " dims");
  TORCH_CHECK(x_scale.layout() == at::Layout::Strided, OPERATOR_NAME_UNIFIED,
              " : Expected xq scale argument to be strided, got layout ",
              x_scale.layout());
  TORCH_CHECK(wq.dim() == 2, OPERATOR_NAME_UNIFIED,
              " : Expected wq argument to be 2D tensor, got ", wq.dim(),
              " dims");
  TORCH_CHECK(wq.layout() == at::Layout::Strided, OPERATOR_NAME_UNIFIED,
              " : Expected wq argument to be strided, got layout ",
              wq.layout());
  TORCH_CHECK(w_scale.dim() == 1 || w_scale.dim() == 2, OPERATOR_NAME_UNIFIED,
              " : Expected wq scale argument to be 1D or 2D tensor, ", "got ",
              w_scale.dim(), " dims");
  TORCH_CHECK(w_scale.layout() == at::Layout::Strided, OPERATOR_NAME_UNIFIED,
              " : Expected wq scale argument to be strided, got layout ",
              w_scale.layout());
  if (bias.numel() > 0) {
    TORCH_CHECK(bias.dim() == 1, OPERATOR_NAME_UNIFIED,
                " : Expected bias argument to be 1D tensor, got ", bias.dim(),
                " dims");
    TORCH_CHECK(bias.layout() == at::Layout::Strided, OPERATOR_NAME_UNIFIED,
  ...
  TORCH_CHECK(xq_sizes.back() == wq.size(1) ||
              ...
              OPERATOR_NAME_UNIFIED, " : Expected xq argument to have ", wq.size(1),
  ...
  for (auto i = 0; i < x_scale_sizes.size(); ++i)
    TORCH_CHECK(x_scale_sizes[i] == xq_sizes[i], OPERATOR_NAME_UNIFIED,
                " : Expected xq scale argument size at position ", i, " to be ",
                xq_sizes[i], ", but got ", x_scale_sizes[i]);
  TORCH_CHECK(w_scale.numel() == wq.size(0), OPERATOR_NAME_UNIFIED,
              " : Expected wq scale argument to have ", wq.size(0),
              " elements, got ", w_scale.numel(), " elements");
  if (bias.numel() > 0) {
    TORCH_CHECK(bias.numel() == wq.size(0), OPERATOR_NAME_UNIFIED,
  ...
  TORCH_CHECK(xq_strides[xq_strides.size() - 1] == 1, OPERATOR_NAME_UNIFIED,
  ...
  for (int i = xq_strides.size() - 3; i >= 0; --i) {
    ...
    TORCH_CHECK(xq_strides[i] == xq_stride_expected, OPERATOR_NAME_UNIFIED,
                " : Expected xq argument in row-major layout");
  }
  TORCH_CHECK(x_scale.is_contiguous(), OPERATOR_NAME_UNIFIED,
              " : Expected xq scale argument to be contiguous");
  const auto wq_strides = wq.strides();
  TORCH_CHECK(wq_strides[0] >= 1 && wq_strides[1] == 1, OPERATOR_NAME_UNIFIED,
              " : Expected wq argument in row-major layout");
  TORCH_CHECK(w_scale.is_contiguous(), OPERATOR_NAME_UNIFIED,
              " : Expected wq scale argument to be contiguous");
  if (bias.numel() > 0) {
    const auto bias_strides = bias.strides();
    TORCH_CHECK(bias_strides[0] == 1, OPERATOR_NAME_UNIFIED,
...
rowwise_scaled_linear_cutlass_unified(
    const at::Tensor& xq, const at::Tensor& x_scale, const at::Tensor& wq,
    const at::Tensor& w_scale, const at::Tensor& bias, at::Tensor& result) {
#if defined(BUILD_ROWWISE_SCALED_LINEAR_CUTLASS)
  // Check inputs.
  if (x_scale.defined()){
  rowwise_scaled_linear_cutlass_check_inputs_unified<ElementA, ElementB>(
      xq, x_scale, wq, w_scale, bias);
  }

  // Squash the input tensors as appropriate.
  const auto xq_sizes = xq.sizes().vec();
  const auto xq_2d = xq; //xq.reshape({-1, xq_sizes.back()});
  const auto x_scale_1d = x_scale; //x_scale.reshape({-1});
  const auto w_scale_1d = w_scale; //w_scale.reshape({-1});
  ...
  dispatch_on_tensor_a_scale_and_tensor_b_scale_unified<ElementA, ElementB>(
...
#else
  TORCH_CHECK_NOT_IMPLEMENTED(false, OPERATOR_NAME_UNIFIED);
  return at::Tensor{};
#endif
}
